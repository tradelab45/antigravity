 ...
 * The share sparkline.
 *
 * Split out of `area-charts-2.tsx` because it draws its own SVG and uses no
 * Recharts at all, while its former neighbours do. Sharing a module meant the
 * home dashboard and the screener pulled 360 KB of Recharts to render a
 * sparkline that never touched it.
 */
              ...
              >
                {(['1D', '5D', '1M'] as const).map((tf) => (
                  <button
                    ...
                    className={`inline-flex min-h-6 min-w-6 items-center justify-center px-1.5 py-1 rounded-md text-[9px] font-black transition-all cursor-pointer ${
                      timeframe === tf
